...
# Define the new implementation for import_voters
...
async def import_voters(request: Request, file: UploadFile = File(...)):
    ...
    try:
        ...
        if not file.filename.endswith('.csv'):
            raise HTTPException(status_code=400, detail="Only .csv files are supported. Please convert your file to CSV format.")
...
# Need to accurately replace the whole function
pattern = r'@app\.post\("/api/admin/import-voters"\)\nasync def import_voters\(request: Request, file: UploadFile = File\(\.\.\.\)\):.*?except Exception as e:\n.*?raise HTTPException\(status_code=500, detail=f"Failed to process file: \{str\(e\)\}"\)'
text = re.sub(pattern, new_impl, text, flags=re.DOTALL)
